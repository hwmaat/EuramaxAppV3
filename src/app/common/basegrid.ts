...
import { DeleteResult, SaveRecordResult, TableResult } from "@app/models/httpresults.model";
...
zenyaFolderId:number;
networkLocation:string;
vibNetworkLocation:string;
    ...
    effect(() => {
      try {
        const s = this.globals.settings();
          ...
          this.zenyaFolderId = Number(s.zenyaFolderId ?? 0);
          this.vibNetworkLocation = s.vibNetworkLocation ?? '';
        } catch (err) {
